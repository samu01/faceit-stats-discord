...
class FaceitData:
    ...
    def __init__(self, api_token):
        ...
        self.base_url_v1 = "https://api.faceit.com/"
    ...
    def player_details_v1_api(self, player_nickname=None):
        if player_nickname is None:
            print("The player nickname cannot be nothing!")
        else:
            api_url = "{}core/v1/nicknames/{}".format(self.base_url_v1, player_nickname)
            
            res = requests.get(api_url, headers=self.headers_v1)
            if res.status_code == 200:
                return json.loads(res.content.decode('utf-8'))
            else:
                return None
    ...
    def player_matches_v1_api(self, player_id=None, game=None, return_items=20):
        if player_id is None:
            print("The player_id cannot be nothing!")
        else:
            if game is None:
                ...
                api_url = "{}stats/api/v1/stats/time/users/{}/games/{}?size={}".format(self.base_url_v1, player_id, game, return_items)
                #print(api_url)
                res = requests.get(api_url, headers=self.headers_v1)
                if res.status_code == 200:
                    return json.loads(res.content.decode('utf-8'))
